...
const PROYECTOS = {
  ...
  async cambiarEstado(id) {
    ...
    try {
      const data = await API.call('actualizarEstadoProyecto', { id, estado: nuevoEstado, refFacturaSigo });
      this.DB.proyectos = data;
      this.render();
...
const ALMACEN = {
    ...
    lista.forEach(i => {
      ...
      tr.innerHTML = `
        ...
        <td style="text-align:center;white-space:nowrap;">
          ...
          <button class="btn-icon btn-icon-del" onclick="ALMACEN.eliminar(${i._rowIndex})" title="Eliminar"><i class="ti ti-trash"></i></button>
        </td>`;
      tbody.appendChild(tr);
    });
  },
  ...
  async eliminar(idx) {
    ...
    try {
      this.DB = await API.call('eliminarItemAlmacen', { rowIndex: idx });
      this.render();
